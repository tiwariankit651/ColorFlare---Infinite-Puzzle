...
export const GameScreen: React.FC<GameScreenProps> = ({ currentLevel, onComplete, onBack, onHintUsed, palette, theme, hintsRemaining = 0, hardModeOn = false }) => {
  ...
  const triggerHintLogic = () => {
    ...
    // Reset grid to initial level layout to clear all user's drawn wrong/conflicting paths
    setGrid(level.grid);
    setHistory([]);
    setMoveHistory([]);
    setMoveCount(0);
    ...
    // Hint guideline trails remain active for 5 seconds so they have plenty of time to trace them
